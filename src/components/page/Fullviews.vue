<style>
  ...
  #fullviews .fullviews-list a:hover {
          ...
          <div class="Fullviews">
            <el-card>
              <div slot="header">
                ...
                <div style="float: right;">
                  <el-date-picker v-model="item.date" type="date" @change="tableDataList(item.lable,item.date)" placeholder="选择日期">
                  </el-date-picker>
                </div>
              </div>
...
<script>
  ...
  export default {
    ...
    mounted() {
      ...
      this.$http.all([this.$http.get('fullviews/list', {
          params: {
            date: date,
            name: '打底裤'
          }
      ...
      tableDataList(name, date) {
        ...
        this.$http
          ...
          .then(response => {
            switch (name) {
              case '打底裤':
                this.listOption[0].data = Object.values(response.data);
                break;
              case '牛仔裤':
                this.listOption[1].data = Object.values(response.data);
                break;
              default:
                this.listOption[2].data = Object.values(response.data);
            }
          })
      },
